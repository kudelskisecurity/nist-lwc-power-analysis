import numpy as np

# Substitution box
sbox_8 = []
sbox = np.array(sbox_8)

# Key permutation step (target index for a given initial index)
NP_TWEAKEY_P = np.array([9, 15, 8, 13, 10, 14, 12, 11, 0, 1, 2, 3, 4, 5, 6, 7])

# Round constants (only the first two are needed)
RC = np.array([0x01, 0x03])
