...
def _attack(capture, enc_oracle, enc, threshold, load_from=None, save_to=None, real_key=None, thresholds=None, fail_fast=False) \
    ...
    #import lascar.plotting
    #lascar.plotting.plot(container.leakages[0])
    ...
    for max_wrong_bytes in range(8):
        ...
        for sel_vector in tqdm(selection_vectors):
            ...
            # print("Possible error bytes:", sel_vector)
            ...
            for value_vector in values_vector_partial:
                # print("\tPossible correct values:", value_vector)
                sel = []
                for i in range(8):
                    if i in sel_vector:
                ...
                # print("\tKey:", key.hex(sep=' ', bytes_per_sep=4), "sel:", sel)
