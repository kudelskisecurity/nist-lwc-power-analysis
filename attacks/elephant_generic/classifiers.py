import numba
import numpy as np
from numba import njit
from lascar import hamming

# Constants

debug = False

sbox_list = [
    
]
# njit requires numpy arrays to work
sbox_ndarray = np.array(sbox_list)
sbox_inverse = [sbox_list.index(i) for i in range(0xff + 1)]


#
# Spongent permutation and its inverse
#

@njit
def reverse_bits(counter) -> int:
    """
    Reverses the order of the bits in the given counter
    :param counter: a 7bit value
    :return:
    """
    # From Elephant reference code (elephant160v2 > spongent.c > retnuoCl)
    return ((counter & 0x01) << 7) | ((counter & 0x02) << 5) | ((counter & 0x04) << 3) \
           | ((counter & 0x08) << 1) | ((counter & 0x10) >> 1) | ((counter & 0x20) >> 3) \
           | ((counter & 0x40) >> 5) | ((counter & 0x80) >> 7)

def counter(lfsr):
    """Iterates the counter for the permutation round"""
    lfsr = (lfsr << 1) | (((0x40 & lfsr) >> 6) ^ ((0x20 & lfsr) >> 5))
    lfsr &= 0x7f
    return lfsr

class ElephantGeneric:
    def __init__(self, state_bits, n_rounds, lfsr_iv, attack_point):
        """
        Initializes a generic elephant model
        :param state_bits: number of bits in the state of the cipher
        :param n_rounds: number of rounds of the spongent permutation
        :param lfsr_iv: initialization vector for the block counter LFSR
        :param attack_point: offset to use when capturing traces on the chipwhisperer
        """
        self.state_bits = state_bits
        self.state_bytes = state_bits // 8
        self.n_rounds = n_rounds
        self.lfsr_iv = lfsr_iv
        self.BLOCK_SIZE = self.state_bytes
        self.attack_point = attack_point

    def pi(self, i):
        return i if i == self.state_bits - 1 else (i * self.state_bits//4) % (self.state_bits - 1)

    def spongent_inverse(self, state, rounds=None):
        if rounds is None: rounds = self.n_rounds

        def counter_inverse(lfsr):
            previous_zero = (((0x40 & lfsr) >> 6) ^ (lfsr & 1))
            lfsr = (lfsr >> 1) | (previous_zero << 6)
            lfsr &= 0x7f
            return lfsr

        def lfsr_n(c, n):
            for i in range(n):
                c = counter(c)
            return c

        def pLayer_inverse(state):
            tmp = [0 for _ in range(self.state_bytes)]
            for i in range(self.state_bytes):
                for j in range(8):
                    # where was the bit sent
                    target_bitno = self.pi(8 * i + j)
                    # retrieve the bit
                    bit = (state[target_bitno // 8] >> target_bitno % 8) & 1
                    # set the bit back
                    tmp[i] ^= bit << j

            return tmp


        c = lfsr_n(self.lfsr_iv, rounds)
        for i in range(rounds):
            c = counter_inverse(c)

            # Permutation
            state = pLayer_inverse(state)

            # SBox
            for i in range(self.state_bytes):
                state[i] = sbox_inverse[state[i]]

            # Add Counter
            state[0] ^= c
            inv_iv = reverse_bits(c)
            state[self.state_bytes - 1] ^= inv_iv

        return state


    def spongent(self, state: bytes, rounds=None):
        if rounds is None: rounds = self.n_rounds

        def pLayer(state):
            tmp = [0 for _ in range(self.state_bytes)]

            for i in range(self.state_bytes):
                for j in range(8):
                    bit = (state[i] >> j) & 1
                    target_bitno = self.pi(8 * i + j)
                    tmp[target_bitno // 8] ^= bit << (target_bitno % 8)

            return tmp

        def spongent_round(iv, state):
            # Add Counter
            state[0] ^= iv
            inv_iv = reverse_bits(iv)
            state[self.state_bytes - 1] ^= inv_iv
            iv = counter(iv)

            # SBox
            for i in range(self.state_bytes):
                state[i] = sbox_ndarray[state[i]]

            print_debug(" --", state)
            # Permutation
            state = pLayer(state)

            return iv, state

        state = list(state)
        iv = self.lfsr_iv

        for i in range(rounds):
            print_debug(i, state)
            iv, state = spongent_round(iv, state)

        print_debug("end", state)
        return bytes(state)


@njit
def classifier(lfsr_iv, state_bytes, state: int, position: int):
    iv = numba.u1(lfsr_iv)
    state = numba.u1(state)

    if position == 0:
        # print(" pos 0 ", state, iv)
        state ^= iv
        # print(" post pos 0 ", state, iv)
    elif position == state_bytes - 1:
        inv = numba.u1(reverse_bits(iv))
        # print("  inv", inv, state)
        state ^= inv

    return sbox_ndarray[state]

@njit
def classifier_ad(lfsr_iv, state_bytes, position, ad: np.ndarray, guess):
    ad_byte = numba.u1(ad[position])
    state = numba.u1(guess) ^ ad_byte
    result = classifier(lfsr_iv, state_bytes, state, position)

    return hamming(result)

def print_debug(round, state):
    if debug:
        print(round, ' '.join(['{:02X}'.format(x) for x in state[::-1]]), sep='\t')




@njit
def rotl(b):
    return (0xff & (b << 1)) | (b >> 7)


@njit
def rotr(b):
    return (0xff & (b << 7)) | (b >> 1)


@njit
def rotl3(b):
    return (0xff & (b << 3)) | (b >> 5)


@njit
def rotr3(b):
    return (0xff & (b << 5)) | (b >> 3)


class JumboModel(ElephantGeneric):
    def __init__(self):
        super().__init__(176, 90, 0x45, 987000)

    def mask_lfsr_step(self, state):
        out = []
        temp = rotl(state[0]) ^ ((state[3] << 7) & 0xff) ^ (state[19] >> 7)

        for i in range(self.state_bytes - 1):
            out.append(state[i + 1])

        out.append(temp)

        return bytes(out)

    def mask_lfsr_goback(self, state):
        out = []

        rotTemp = ((state[2] << 7) & 0xff) ^ (state[18] >> 7) ^ state[21]
        temp = rotr(rotTemp)

        out.append(temp)
        for i in range(1, self.state_bytes):
            out.append(state[i - 1])

        return bytes(out)


class DumboModel(ElephantGeneric):
    def __init__(self):
        super().__init__(160, 80, 0x75, 974000)

    def mask_lfsr_step(self, state):
        out = []
        temp = rotl(state[0]) ^ ((state[3] << 7) & 0xff) ^ (state[19] >> 7)

        for i in range(self.state_bytes - 1):
            out.append(state[i + 1])

        out.append(temp)

        return bytes(out)

    def mask_lfsr_goback(self, state):
        out = []

        rotTemp = ((state[2] << 7) & 0xff) ^ (state[12] >> 7) ^ state[19]
        temp = rotr3(rotTemp)

        out.append(temp)
        for i in range(1, self.state_bytes):
            out.append(state[i - 1])

        return bytes(out)



